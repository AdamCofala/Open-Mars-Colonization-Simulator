...
void Renderer::RenderStructures(const Map& map) {
    ...
    const bool placingPipe = (r_selectedBuildingType == 1 && r_selectedTile.x >= 0
        && r_selectedTile.y >= 0);
    ...
    for (const auto* s : sorted) {
        ...
        if (s->isPipe()) {
            ...
            // During pipe placement, neighboring pipes get a virtual mask
...
void Renderer::RenderSelected(const Map& map, Vector2 offset, Color tint) {
    ...
    if (r_selectedBuildingType >= 0) {
        ...
        // Duch rury – nie tworzymy już tymczasowego obiektu, używamy bezpośrednio RenderPipe
        if (r_selectedBuildingType == 1) {
            ...
            // Dla pozostałych budynków – tymczasowy obiekt tylko do pobrania tekstury
